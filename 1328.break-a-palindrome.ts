/*
 * @lc app=leetcode.cn id=1328 lang=typescript
 * @lcpr version=30204
 *
 * [1328] 破坏回文串
 *
...
function breakPalindrome(palindrome: string): string {
    let end = Math.floor(palindrome.length / 2);
    if (end == 0) return String();
    for (let index = 0; index < end; index++) {
        if (palindrome[index] == 'a') continue;
        palindrome = palindrome.substring(0, index) +
            'a' + palindrome.substring(index + 1, palindrome.length);
        return palindrome;
    }
