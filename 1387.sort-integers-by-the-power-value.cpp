...
public:
    int getKth(int lo, int hi, int k) {
        ...
        for (int i = lo; i <= hi; i++)
            weight[calcWeight(i)].push_back(i);
        for (auto &[wgt, val] : weight) {
            if (k > val.size()) {
                k -= val.size();
                continue;
            }
    ...
    // // 试了一下直接用ranges::sort自带的projection来一步到位排序
    // // 看起来内存占用因为省下了一个map所以好了很多，但是运行效率有点一坨
    // int getKth(int lo, int hi, int k) {
    //     vector<int> num;
    //     for (int i = lo; i <= hi; i++)
    //         num.push_back(i);
    ...
    //         return ans;
    //     };
    //     ranges::sort(num, less(), [&](int i) -> tuple<int, int> {
    //         return tuple{calcWeight(i), i};
    //     });
    //     return num[k-1];
    // }
};
